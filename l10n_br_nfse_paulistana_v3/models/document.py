...
from erpbrasil.assinatura import assinatura as erpbrasil_assinatura
...
def filter_paulistana(record):
    if record.company_id.provedor_nfse == "paulistanav3":
        return True
    return False


def only_digits(value):
    """Retorna somente os dígitos de `value` como string.

    Se `value` for None, retorna string vazia.
    """
    if value is None:
        return ""
...
class Document(models.Model):
    ...
    def _serialize(self, edocs):
        ...
        for record in self.filtered(filter_oca_nfse).filtered(filter_paulistana):
            edocs.append(record.serialize_nfse_paulistanav3())
        return edocs

    def serialize_nfse_paulistanav3(self):
        dados_lote_rps = self._prepare_lote_rps()
        dados_servico = self._prepare_dados_servico()
        lote_rps = PedidoEnvioLoteRPS(
            Cabecalho=self._serialize_cabecalhov3(dados_lote_rps),
            RPS=[self._serialize_lote_rpsv3(dados_lote_rps, dados_servico)],
        )
        return lote_rps

    def _serialize_cabecalhov3(self, dados_lote_rps):
    ...
    def _serialize_lote_rpsv3(self, dados_lote_rps, dados_servico):
        dados_tomador = self._prepare_dados_tomador()
        return tpRPS(
            Assinatura=self.assinatura_rpsv3(
            ...
            ValorFinalCobrado=self.convert_type_nfselib(
                tpRPS, "ValorFinalCobrado", dados_servico["valor_final_cobrado"]
            ),
            ValorIPI=self.convert_type_nfselib(
                tpRPS, "ValorIPI", dados_servico["valor_ipi"]
            ),
            ...
            NBS=self.convert_type_nfselib(
                tpRPS, "NBS", only_digits(dados_servico["codigo_nbs"])
            ),
            cLocPrestacao=self.convert_type_nfselib(
                tpRPS, "cLocPrestacao", dados_servico["municipio_prestacao_servico"]
            ),
            IBSCBS=tpIBSCBS(
                finNFSe=self.convert_type_nfselib(
                    tpIBSCBS, "finNFSe", dados_servico["finalidade_nfse"]
                ),
                indFinal=self.convert_type_nfselib(
                    tpIBSCBS, "indFinal", dados_servico["uso_consumo"]
                ),
                cIndOp=self.convert_type_nfselib(
                    tpIBSCBS, "cIndOp", dados_servico["codigo_indicador_operacao"]
                ),
                indDest=self.convert_type_nfselib(
                    tpIBSCBS, "indDest", dados_servico["indicador_destinatario"]
                ),
                valores=tpValores(
                    trib=tpTrib(
                        gIBSCBS=tpGIBSCBS(
    ...
    def _serialize_rpsv3(self, dados):
        return tpRPS(
            InscricaoMunicipalTomador=self.convert_type_nfselib(
    ...
    def assinatura_rpsv3(self, dados_lote_rps, dados_servico, dados_tomador):
        assinatura = ""

        assinatura += dados_lote_rps["inscricao_municipal"].zfill(12)
        ...
        # Versão 2 da NFS-e paulistana utiliza SHA1 com PKCS#1 v1.5
        # Alterado para ja armazenar assinado e nao assinar no envio
        certificado = self.env.company._get_br_ecertificate()
        assinador = erpbrasil_assinatura.Assinatura(certificado)
        assinatura_rsasha1 = assinador.sign_pkcs1v15_sha1(assinatura.encode("utf-8"))
        return assinatura_rsasha1
        ...
        for record in self.filtered(filter_oca_nfse).filtered(filter_paulistana):
            ...
            if not protocolo:
                for edoc in record.serialize():
                    processo = None
                    # ajuste para indicar que é sincrono
                    # for p in processador.processar_documento(edoc):
                    for p in processador.processar_documento(edoc, envio_sincrono=True):
                        processo = p
                        ...
                        if processo.webservice in CONSULTA_LOTE:
                            if processo.resposta.Cabecalho.Sucesso:
                                ...
                                # Save response XML in the authorization event
                                if not record.authorization_event_id:
                                    record.authorization_event_id = (
                                        record.event_ids.create_event_save_xml(
                                            ...
                                            event_type="0",
                                            xml_file=getattr(
                                                processo, "envio_xml", processo.retorno
                                            ),
                                            document_id=record,
                                        )
                                    )
                                record.authorization_event_id.set_done(
                                    status_code=4,
                                    response=vals.get("status_name", ""),
                                    protocol_date=vals.get("authorization_date"),
                        ...
                        if processo.webservice in ENVIO_LOTE_RPS:
                            if retorno:
                                # Persist response XML in the authorization event
                                ...
                                # Ensure we have an authorization event
                                # and save the response XML
                                if not record.authorization_event_id:
                                    # try to reuse envio_xml from the process
                                    # if available, otherwise use edoc/retorno
                                    xml_req = (
                                        getattr(processo, "envio_xml", None)
                                        or getattr(processo, "envio", None)
                                        or edoc
                                    )
                                    record.authorization_event_id = (
                                        record.event_ids.create_event_save_xml(
                                ...
                                # Save the response XML and mark the event done
                                record.authorization_event_id.set_done(
                                    status_code=vals.get("status_code", 3),
                                    response=vals.get("status_name", ""),
                                    protocol_date=vals.get("authorization_date"),
                                    protocol_number=protocolo,
                                    file_response_xml=processo.retorno,
